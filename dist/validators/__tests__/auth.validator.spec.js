...
describe('Auth Validators', () => {
    ...
    describe('registerSchema', () => {
        ...
        it('rejects username less than 3 characters', () => {
            ...
            const result = auth_validator_1.registerSchema.safeParse(invalid);
    ...
    describe('loginSchema', () => {
        it('accepts valid login input', () => {
            const valid = { username: 'testuser', password: 'password123' };
            const result = auth_validator_1.loginSchema.safeParse(valid);
            expect(result.success).toBe(true);
        });
        it('validates that username and password are required', () => {
            // Test missing username
            const missingUsername = { password: 'password123' };
            const result1 = auth_validator_1.loginSchema.safeParse(missingUsername);
            expect(result1.success).toBe(false);
            // Test missing password
            const missingPassword = { username: 'testuser' };
            const result = auth_validator_1.loginSchema.safeParse(missingPassword);
            expect(result.success).toBe(false);
        });
        it('allows login with simple password (no uppercase/number requirement)', () => {
            ...
            const result = auth_validator_1.loginSchema.safeParse(input);
            expect(result.success).toBe(true);
        });
    });
});
